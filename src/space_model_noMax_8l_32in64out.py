    ...
    def __init__(self, name):
        ...
        flat_model = UNetLike([
            ...
            nn.Sequential(
                ...
                Conv2d(64, 128, (4, 4), 4),  nn.PReLU(),  # 10, 26, 26
                #Conv2d(10, 10, (3, 3), 1), nn.Dropout(), nn.PReLU(),  # 10, 24, 24
                # preserving_dimensions(Conv2d, 10, 10), nn.Dropout(), nn.PReLU()
            ),
            nn.Sequential(
                Conv2d(128, 256, (4, 4), 4),  nn.PReLU(),  # 10, 6, 6
            ),

        ], [

            nn.Sequential(
                preserving_dimensions(Conv2d, 256, 128),  nn.PReLU(),
                Repeat('b c x y -> b c (x dx) (y dy)', dx=4, dy=4),  # 10, 24, 24
                ConvTranspose2d(128, 128, (3, 3)), nn.PReLU()
            ...
            nn.Sequential(
                preserving_dimensions(Conv2d, 32, 1)
            )
