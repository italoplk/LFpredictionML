...
def read_LF(path):
    ...
    print(img_normalizada.min())
    print(img_normalizada.max())
    plt.imsave("/scratch/savetest.png", np.clip(img_normalizada, 0, 1))
    try:
        return rearrange(img_normalizada, '(s u) (t v) c -> c s t u v', s=13, t=13)[:1, :, :, :, :]
    except EinopsError:
